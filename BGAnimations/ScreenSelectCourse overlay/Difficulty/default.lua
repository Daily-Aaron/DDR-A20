...
local function DrawDiffListItem(diff)
  local DifficultyListItem = Def.ActorFrame{
    ...
    Def.BitmapText{
      Font="_geo 957 Bold",
      Name="Score";
      InitCommand=function(s) s:draworder(5):xy(74,-1):zoom(1.1):halign(1):maxwidth(80) end,
      SetCommand=function(self)
        self:settext("")
    ...
    Def.ActorFrame{
      ...
      Def.Quad{
        Name="Grade";
      InitCommand=function(s) s:draworder(5):visible(false):zoom(0.7):x(4) end,
      SetCommand=function(self)
        local st=GAMESTATE:GetCurrentStyle():GetStepsType();
        local song=GAMESTATE:GetCurrentSong();
        if song then
            ...
            if scores[1] then
              ...
              if scores[1]:GetScore()>1  then
                if topgrade == 'Grade_Failed' then
                  self:LoadBackground(THEME:GetPathG("","ScreenStageInformation/Grade Failed"));
                else
                  self:LoadBackground(THEME:GetPathG("ScreenStageInformation/Grade",ToEnumShortString(tier)));
                end;
                self:visible(true)
              else
                self:visible(false)
...
return Def.ActorFrame{
  CurrentSongChangedMessageCommand=function(s) s:finishtweening():queuecommand("Set") end,
    ...
    Def.ActorFrame{
        Def.ActorFrame{
            ...
            Def.Sprite{
                ...
                InitCommand=function(s) s:xy(pn==PLAYER_1 and 12 or 20,-46) end,
    ...
    Def.ActorFrame{
      ...
      Def.Sprite{
           Texture=ddrgame.."dif_windo_cursor_line",
        InitCommand=function(s) s:zoomx(0.96):zoomy(1):diffuse(color("#82fefe")):thump(1):effectmagnitude(1.02,1,0):effectperiod(0.6):visible(false) end,
