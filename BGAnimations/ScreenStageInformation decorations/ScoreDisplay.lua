...
t[#t+1]=Def.ActorFrame{
	LoadActor(THEME:GetPathB("","_blue doors/"..ddrgame.."cosh_choice_difficulty_base.png"))..{
		...
		OnCommand=function(self)
			...
			self:x(pn == PLAYER_1 and SCREEN_LEFT+OffsetX-1 or SCREEN_RIGHT-OffsetX+1);
		end;
	};
	LoadActor(THEME:GetPathB("","_blue doors/"..ddrgame.."cosh_choice_best_score.png"))..{
		InitCommand=function(self)
			self:x(pn == PLAYER_1 and SCREEN_LEFT-OffsetX or SCREEN_RIGHT+OffsetX)
			self:y(SCREEN_CENTER_Y+203):zoom(1);
		end;
	OnCommand=function(self)
		self:sleep(SleepOffset+0.2):linear(0.05)
		self:x(pn == PLAYER_1 and SCREEN_LEFT+OffsetX or SCREEN_RIGHT-OffsetX);
	...
	--[[LoadActor(THEME:GetPathB("","_blue doors/"..ddrgame.."cosh_choice_target_score.png"))..{
		InitCommand=function(self)
			self:x(pn == PLAYER_1 and SCREEN_LEFT-OffsetX or SCREEN_RIGHT+OffsetX)
	...
	InitCommand=function(s) s:zoom(0.25):shadowlength(1)
	s:x(pn == PLAYER_1 and SCREEN_LEFT+167-OffsetX or SCREEN_RIGHT+167+OffsetX)
	s:y(SCREEN_CENTER_Y+195):horizalign(center):draworder(2) end,
	OnCommand=function(self)
			local SongOrCourse, StepsOrTrail;
			if GAMESTATE:IsCourseMode() then
				SongOrCourse = GAMESTATE:GetCurrentCourse();
...
--Grade	
	...
	s:y(SCREEN_CENTER_Y+193):horizalign(center):draworder(2) end,
		...
		OnCommand=function(self)
			...
			self:sleep(SleepOffset+0.2):linear(0.05)
			self:x(pn == PLAYER_1 and SCREEN_LEFT+OffsetX+152 or SCREEN_RIGHT-OffsetX+152);
		end;
	};
	...
	LoadFont("_helveticaneuelt pro 65 md 24px") .. {
		...
		s:y(SCREEN_CENTER_Y+228):horizalign(left):strokecolor(Color("Outline")) end,
		OnCommand=function(self)
			self:sleep(SleepOffset+0.2):linear(0.05)
			self:x(pn == PLAYER_1 and SCREEN_LEFT+OffsetX-177 or SCREEN_RIGHT-OffsetX-177);
		end;
	...
	LoadActor(regionPath.."Region.lua")..{
		...
		s:y(SCREEN_CENTER_Y+228):zoom(1) end,
...
for _,pn in pairs(GAMESTATE:GetEnabledPlayers()) do
	t[#t+1] = Def.Sprite{
		Texture ="diff 1x6";
		InitCommand=function(s) s:y(SCREEN_CENTER_Y+150):horizalign(center)
