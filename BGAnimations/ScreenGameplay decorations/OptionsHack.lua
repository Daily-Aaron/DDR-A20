...
function InitCoverPos(self, player, CoverPosition, pos, Mode, TwoCoverMode)
	if not GAMESTATE:IsCourseMode() then
		local PlayerUID = PROFILEMAN:GetProfile(player):GetGUID(); 
		(cmd(zoom,1.67;x,pos;y,SCREEN_CENTER_Y;draworder,101))(self);
		...
		if string.find(string.lower(OptionString),"reverse") or CourseSongTrailHasReverse then
			...
			if Mode == "Hidden+" then
				...
				if string.find(FileName, "Upper") then
					self:zoomy(-1.5);
				end
...
function AppearancePlusMain(pn)
	...
	elseif MyValue == "Hidden+" then
		...
		if GAMESTATE:GetCurrentStyle():GetStepsType()=="StepsType_Dance_Single" then
			if not GAMESTATE:PlayerIsUsingModifier(player,'reverse') then
				AddCoverLayer(ddrgame.."UpperCoverSingle", player, CoverPosition, pos, "Hidden+",false);
			else
				AddCoverLayer(ddrgame.."LowerCoverSingle", player, CoverPosition, pos, "Hidden+",false);
			end
		elseif GAMESTATE:GetCurrentStyle():GetStepsType()=="StepsType_Dance_Double" then
			if not GAMESTATE:PlayerIsUsingModifier(player,'reverse') then
				AddCoverLayer(ddrgame.."UpperCoverDouble", player, CoverPosition, pos, "Hidden+",false);
			else
				AddCoverLayer(ddrgame.."LowerCoverDouble", player, CoverPosition, pos, "Hidden+",false);
			end
		end
	elseif MyValue == "Sudden+" then
		OptionString = string.gsub(OptionString, "(Sudden,)", "");
		...
		if GAMESTATE:GetCurrentStyle():GetStepsType()=="StepsType_Dance_Single" then
			if not GAMESTATE:PlayerIsUsingModifier(player,'reverse') then
				AddCoverLayer(ddrgame.."LowerCoverSingle", player, CoverPosition, pos, "Sudden+",false);
			else
				AddCoverLayer(ddrgame.."UpperCoverSingle", player, CoverPosition, pos, "Sudden+",false);
			end
		elseif GAMESTATE:GetCurrentStyle():GetStepsType()=="StepsType_Dance_Double" then
			if not GAMESTATE:PlayerIsUsingModifier(player,'reverse') then
				AddCoverLayer(ddrgame.."LowerCoverDouble", player, CoverPosition, pos, "Sudden+",false);
			else
				AddCoverLayer(ddrgame.."UpperCoverDouble", player, CoverPosition, pos, "Sudden+",false);
			end
		end
	elseif MyValue == "Hidden+&Sudden+" then
		...
		if GAMESTATE:GetCurrentStyle():GetStepsType()=="StepsType_Dance_Single" then
			if not GAMESTATE:PlayerIsUsingModifier(player,'reverse') then
				AddCoverLayer(ddrgame.."UpperCoverSingle", player, CoverPosition, pos, "Hidden+",true);
				CoverPosition = 0;
				AddCoverLayer(ddrgame.."LowerCoverSingle", player, CoverPosition, pos, "Sudden+",true);
			else
				AddCoverLayer(ddrgame.."UpperCoverSingle", player, CoverPosition, pos, "Sudden+",true);
				CoverPosition = 0;
				AddCoverLayer(ddrgame.."LowerCoverSingle", player, CoverPosition, pos, "Hidden+",true);
			end
		elseif GAMESTATE:GetCurrentStyle():GetStepsType()=="StepsType_Dance_Double" then
			if not GAMESTATE:PlayerIsUsingModifier(player,'reverse') then
				AddCoverLayer(ddrgame.."UpperCoverDouble", player, CoverPosition, pos, "Hidden+",true);
				CoverPosition = 0;
				AddCoverLayer(ddrgame.."LowerCoverDouble", player, CoverPosition, pos, "Sudden+",true);
			else
				AddCoverLayer(ddrgame.."UpperCoverDouble", player, CoverPosition, pos, "Sudden+",true);
				CoverPosition = 0;
				AddCoverLayer(ddrgame.."LowerCoverDouble", player, CoverPosition, pos, "Hidden+",true);
			end
		end
	else
		OptionString = string.gsub(OptionString, "(Sudden,)", "");
