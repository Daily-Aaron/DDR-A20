local Image = Def.ActorFrame{
	LoadActor(ddrgame.."pad") ..{
		InitCommand=cmd(diffusealpha,0;zoomx,1;x,-18);
		OnCommand=cmd(zoom,0;sleep,0.5;linear,0.1;diffusealpha,1.0;zoom,1;smooth,0.1;zoom,0.9;smooth,0.1;zoom,1);
		GainFocusCommand=cmd(linear,0.2;y,-16);
		LoseFocusCommand=cmd(linear,0.2;y,51);
	};
	LoadActor(ddrgame.."pad") ..{
		InitCommand=cmd(blend,Blend.Add;diffusealpha,0;zoomx,1;x,-18);
		GainFocusCommand=cmd(y,-16;diffusealpha,0;linear,0.01;smooth,0.3;diffusealpha,1;diffuseramp;effectcolor1,color("1,1,1,1");effectcolor2,color("1,1,1,0");effectperiod,0.45);
		LoseFocusCommand=cmd(diffusealpha,0;stopeffect;y,51);
	};
	LoadActor(ddrgame.."character") ..{
		InitCommand=cmd(diffusealpha,0;zoomx,1;x,-17);
		OnCommand=cmd(sleep,0.6;linear,0.1;diffusealpha,1;zoomy,0.8;linear,0.1;zoomy,1;zoomx,1.2;linear,0.1;zoomx,1);
		GainFocusCommand=cmd(linear,0.2;y,-14);
		LoseFocusCommand=cmd(linear,0.2;y,51);
	};
	LoadActor(THEME:GetPathG("","_blank"))..{
		...
		PlayCommand=function(self)
			...
			SOUND:PlayOnce(THEME:GetPathS("","Announcer/ScreenSelectStyle Versus"));
			end;
		end;
	};
};
